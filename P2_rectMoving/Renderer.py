	...
	def draw(self, jobData):
		...
		for n in range(0, len(jobData)):
			...
			if(jobData[n].state == "Q"):
				rectQ = pygame.Rect(widthQandH + X, 850 + Y, (jobData[n].node * jobData[n].core)/3,( -jobData[n].wallrequest / 900))
			...
			if(jobData[n].state == "H"):
				rectH = pygame.Rect(widthQandH + X, 850 + Y, (jobData[n].node * jobData[n].core)/3,( -jobData[n].wallrequest / 900))
			...
			if(jobData[n].state == "R"):
				rect = pygame.Rect(widthR + X, 300 + Y, (jobData[n].node * jobData[n].core), (jobData[n].wallrequest / 900)+(jobData[n].starttime)/900)
				pygame.draw.rect(self.screen, self.hexColor(jobData[n].owner), rect)
				rectInside = pygame.Rect(widthR + X, 300 + Y, (jobData[n].node * jobData[n].core), (jobData[n].starttime)/900) 
